...
namespace mylib {
    ...
    namespace details {
        ...
        struct alignas(queue_align) queue_head
        {
            ...
            std::atomic_size_t shared_counter = 0;
        };
        ...
        template<typename T>
        class queue_unit : private queue_head
        {
        public:
            using value_type = T;
            ...
            static std::unique_ptr<queue_unit> make(std::size_t capacity) {
                ...
                // noexcept
                queue_unit* const ptr = new(raw) queue_unit(capacity);
                return std::unique_ptr<queue_unit>(ptr);
            }

            void operator delete(queue_unit* ptr, std::destroying_delete_t) noexcept {
        ...
        private:
            explicit queue_unit(std::size_t capacity) noexcept : queue_head(capacity) {}

            static cell_type* to_storage_ptr(queue_unit* ptr) noexcept {
    ...
    public:
        using value_type = T;
        using queue_unit_type = details::queue_unit<value_type>;
    ...
    public:
        ...
        [[nodiscard]]
        bool enqueue(value_type&& v) noexcept {
            if (queue_unit_type* const cur = this->enqueue_ptr.load(std::memory_order_relaxed)) {
        ...
        values_view fetch_values() noexcept {
            while (stealing.test_and_set(std::memory_order_acq_rel)) std::this_thread::yield();
        ...
        values_view steal(queue& other) noexcept {
            if (other.stealing.test_and_set(std::memory_order_acq_rel)) {
                return values_view();
            } else {
                other.switch_enqueue_handle(this->dequeue_handle);
                other.stealing.clear(std::memory_order_release);
            }
            return wait_for_exclusive_values();
        }
    ...
    private:
        ...
        void switch_enqueue_handle(queue_unit_handle_type& new_enqueue_handle) noexcept {
            ...
            enqueue_ptr.store(this->enqueue_handle.get(), std::memory_order_release);
        }
        ...
        struct alignas(details::queue_align) {
            ...
            std::atomic<queue_unit_type*> enqueue_ptr;
            std::atomic_flag stealing;
        };
